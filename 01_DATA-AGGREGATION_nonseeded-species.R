...
##              that were not in the seed mix (did not arrived from the pool)
##              
...
# Species from seed mix (does not include codes for unkown sedges: CxGEN1 - CXGEN5)
data.seed.nocx <- as.character(data.seed[data.seed$high.no.cxgen == "yes", "Species"])
not.seeded.nocx <- data.spe[ ! data.spe$Species %in% data.seed.nocx, ]


#### 2. CALCULATE RICHNESS AT EACH SCALE ####

# 2.1) CALCULATE MEAN RICHNESS AT 1-m2 for not.seeded.all
# Subset data to only include 1-m2 qudrat occurrences
m1_scale <- not.seeded.nocx %>%
  filter(Scale == "1") 
...
SAR_nonseededspecies_nocx <- data_1m_rich %>%
  left_join(data_5m_rich, by = c("Site", "Half")) %>%
  left_join(data_transect_rich, by = c("Site", "Half")) %>%
  left_join(site_scale, by = c("Site", "Half"))  %>%
  gather(scale, richness.nonseeded.nocx, rich1m:siteRich)

# Import treatment ID and site scale area
trt.data <- read.csv("raw_data/CLE_treatment_codes.csv")

SAR_nonseededspecies_nocx <- SAR_nonseededspecies_nocx %>%
  left_join(trt.data, by = c("Site", "Half", "scale")) %>%
  select(-c(Half, scale))

# Output data for analysis
write.csv(SAR_nonseededspecies_nocx, "output/SAR_data_nonseededspecies_nocx.csv")
